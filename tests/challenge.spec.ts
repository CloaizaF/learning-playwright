...
test.describe("Verify Checkout Workflow", () => {
  ...
  test("Verify Buy Now Pay Later", async ({ page, headless }) => {
    ...
    ).toHaveCSS("background-color", "rgb(51, 131, 51)");
    ...
    await page
      .getByTestId("monthly-installments")
    ...
    headless
      ? await test.step("visual test", async () => {
          await expect(page).toHaveScreenshot("chekcout.png", {
            mask: [page.getByTitle("Practice Software Testing - Toolshop")],
          });
        })
      : console.log("Running om Headed mode, no screenshot comparison");
  });
});

test.describe("Verify API", () => {
  test("Verify GET /products/{id}", async ({ request }) => {
